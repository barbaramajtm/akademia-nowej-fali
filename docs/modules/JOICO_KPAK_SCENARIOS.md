# Scenariusze — JOICO K-PAK

| ID | Tytuł |
|----|-------|
| `joico-kpak-co-to` | Czym jest JOICO K-PAK? |
| `joico-kpak-owrn` | O-W-R-N: mapa czterech etapów |
| `joico-kpak-o-cel` | Oczyszczanie: po co ten etap |
| `joico-kpak-o-dlugosci` | Oczyszczanie: długości vs skóra głowy |
| `joico-kpak-o-czas` | Oczyszczanie: czas, spłukanie, ręcznik |
| `joico-kpak-w-cel` | Wygładzanie: po co ten etap |
| `joico-kpak-w-jak` | Wygładzanie: jak nakładać |
| `joico-kpak-r-cel` | Rekonstrukcja: po co ten etap |
| `joico-kpak-r-jak` | Rekonstrukcja: jak nakładać |
| `joico-kpak-powtorzenie-2-3` | Powtórka kroków 2 i 3 |
| `joico-kpak-tonacja-po-3` | Tonacja po kroku 3 |
| `joico-kpak-n-cel` | Nawilżenie: po co ten etap |
| `joico-kpak-n-jak` | Nawilżenie: jak nakładać |
| `joico-kpak-procedura` | Procedura: cała kolejność bez kartki |
| `joico-kpak-scenki-salonu` | Scenki przy fotelu — K-PAK |
| `joico-kpak-rozmowa` | Rozmowa i technologie |

Kolejność w akademii: **JOICO K-PAK → Babilon Intelligent → …** (pozostałe moduły na razie zablokowane).

## Akcent dydaktyczny

- **Bez wklejonej kartki instrukcji** na ekranie lekcji — kartka była ściągą; uczymy podpunktami.
- Każdy etap O / W / R / N: najpierw **po co**, potem **jak** (aplikacja, czas, skóra vs długości).
- **Kluczowe pytanie klientki (O):** „Nakładasz na całą długość — co w tym niezwykłego?”  
  Odpowiedź: zwykły szampon idzie głównie na **skórę głowy**; Clarifying czyści **długości z osadów**, żeby pielęgnacja weszła równo.
